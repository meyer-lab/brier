...
Tables are [polars](https://github.com/pola-rs/polars)/parquet, arrays are npz.
`arrays/` and `model_state/` are written **immediately** per replicate (durable
...
`log_replicate` pulls the replicate index / seed / membership from the `ResamplePlan`. There is no fixed metric schema. Column names are whatever you pass. `bs.select(X, idx)` slices ndarrays (`X[idx]`) or lists of bags (`[X[i] for i in idx]`). Which procedure to pick, and the variable-size draw primitives beneath them, are in [DOCS.md #2](DOCS.md#2-bootstrap-procedures).
...
Represent any "same resample, different fit" axis as **separate runs sharing the `bootstrap_seed`**: identical seeds give identical per-replicate `fit_indices`, so the runs' replicates are row-joinable on `replicate_idx` for paired comparisons. Details in [DOCS #7](DOCS.md#7-reproducibility-paired-runs-storage).
